...
// ✅ Correct path (since this file is inside the 'admin' folder)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $password === '') {
        ...
        // ✅ Fixed: use the correct table and existing columns
        ...
        if ($res && mysqli_num_rows($res) === 1) {
            ...
            // ✅ Compare plain text password (matches your database)
            if ($password === $admin['password']) {
                // Login success
                $_SESSION['admin_id'] = $admin['id'];
                $_SESSION['admin_name'] = $admin['username']; // ✅ no 'name' column, so use username

                // ✅ Redirect to dashboard inside admin folder
